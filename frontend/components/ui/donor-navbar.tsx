...
export function DonorNavbar() {
  ...
  const markAsRead = async (id: number) => {
    ...
    if (!token) return;
    try {
      await fetch(`/api/v1/donors/me/notifications/${id}/read`, {
        method: "PATCH",
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const markAllAsRead = async () => {
    ...
    if (!token) return;
    try {
      await fetch("/api/v1/donors/me/notifications/read-all", {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
    ...
    <header className="fixed inset-x-0 top-0 z-50 w-full border-b border-bone-50/10 bg-ink-950/95 backdrop-blur-xl">
      <div className="container-shell flex h-16 items-center justify-between md:h-[68px]">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link to="/dashboard/donor" className="flex items-center gap-2.5" aria-label="Blood Coord home">
        ...
        {/* Right: Mobile icons (notification + profile) */}
            ...
            <AnimatePresence>
              {showNotifications && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -10, scale: 0.95 }}
                  transition={{ duration: 0.15 }}
                  className="absolute right-0 top-full mt-2 w-72 overflow-hidden rounded-2xl border border-ink-900/10 bg-bone-50 shadow-xl sm:w-80"
                >
                  <div className="flex items-center justify-between border-b border-ink-900/10 px-4 py-3">
                    <h3 className="font-display text-sm font-semibold text-ink-950">Notifications</h3>
                    {unreadCount > 0 && (
